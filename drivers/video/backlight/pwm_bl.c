...
int panel_is_TF600T = 1;    //display panel is TF600T
...
static tegra_dc_bl_output cardhu_bl_output_measured_TF600T = {
        0, 28, 28, 28, 28, 28, 28, 28,
        28, 28, 28, 28, 28, 28, 28, 28,
        28, 28, 28, 28, 28, 28, 28, 28,
        28, 28, 28, 28, 28, 29, 30, 31,
...
static p_tegra_dc_bl_output bl_output_TF600T = cardhu_bl_output_measured_TF600T;
	...
	if (brightness == 0) {
		...
		if (tegra3_get_project_id()==TEGRA3_PROJECT_ME301T) {
			...
			if (brightness > 255) {
                ...
                //TF600T need to map the brightness below 10% to 10% (28)
                if (panel_is_TF600T) {
                        if (WARN_ON(ARRAY_SIZE(cardhu_bl_output_measured_TF600T) != 256))
                                pr_err("bl_output array does not have 256 elements\n");
                        ...
                        if (brightness > 255) {
                                ...
                                brightness = bl_output_TF600T[brightness];
                        }
                }
